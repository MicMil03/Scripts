...
# This will backup the Thunder drive to RaidNAS on a daily basis.
...
# Backup all department shares
if [ -e /Volumes/Backup ] ; then
	date +"%m/%d/%Y %H:%M:%S Starting Backup..." >> $logfile
	rsync -a /Volumes/Thunder/Art /Volumes/Backup
	date +"%m/%d/%Y %H:%M:%S Art Backup complete." >> $logfile	
	rsync -a /Volumes/Thunder/AV /Volumes/Backup
	date +"%m/%d/%Y %H:%M:%S AV Backup complete." >> $logfile
	rsync -a /Volumes/Thunder/DeployStudio /Volumes/Backup
	date +"%m/%d/%Y %H:%M:%S DeployStudio Backup complete." >> $logfile
	rsync -a /Volumes/Thunder/Guidance /Volumes/Backup
	date +"%m/%d/%Y %H:%M:%S Guidance Backup complete." >> $logfile	
	rsync -a /Volumes/Thunder/Science /Volumes/Backup
	date +"%m/%d/%Y %H:%M:%S Science Backup complete." >> $logfile	
	rsync -a /Volumes/Thunder/Spanish123 /Volumes/Backup
	date +"%m/%d/%Y %H:%M:%S Spanish123 Backup complete." >> $logfile	
	rsync -a /Volumes/Thunder/SpanishAP /Volumes/Backup
